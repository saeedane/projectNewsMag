...
class ProductSystemReport extends ProductPanelAbstract {

	public $id = 'report';

	public $check_remote_duration = HOUR_IN_SECONDS;
	...
	public function enqueue() {
		wp_enqueue_style( 'ak-system-report-styles', AK_FRAMEWORK_URL . '/assets/css/panel/ak-system-report.css', array(), AK_FRAMEWORK_VERSION );
		wp_style_add_data( 'ak-system-report-styles', 'rtl', 'replace' );

		wp_enqueue_script( 'ak-system-report-scripts', AK_FRAMEWORK_URL . '/assets/js/panel/ak-system-report.js', array(), AK_FRAMEWORK_VERSION );
	}
...
<div class="ak-panel-box-container ak-clearfix">

	<div class="ak-panel-box">
		<span class="ak-panel-box-header">
				<?php
				if ( isset( $box['box-settings']['icon'] ) ) {
					echo ak_get_icon( $box['box-settings']['icon'] );
				}

				if ( isset( $box['box-settings']['header'] ) ) {
					ak_sanitize_echo( $box['box-settings']['header'] ); // escaped before
				}
				?>
		</span>

		<div class="ak-panel-box-description ak-panel-box-description-fluid">
				<?php if ( ! empty( $box['items'] ) ) : ?>
			<div class="ak-pages-list-wrapper">
					<?php
					foreach ( $box['items'] as $item ) :
						$have_label          = ! empty( $item['label'] );
						$this->item_settings = $this->get_item_settings( $item );
						?>
				<div class="ak-pages-list-item
						<?php
						if ( ! empty( $item['class'] ) ) {
						...
						if ( $have_label ) :
							?>
					<div class="ak-pages-list-title">
							<?php

							if ( isset( $item['before_label'] ) ) {
								ak_sanitize_echo( $item['before_label'] ); // escaped before
							}

							ak_sanitize_echo( $item['label'] ); // escaped before

							if ( isset( $item['after_label'] ) ) {
								ak_sanitize_echo( $item['after_label'] ); // escaped before
							}
							?>
					</div>
				<?php endif; ?>
					<div class="ak-pages-list-data
						<?php
						if ( ! $have_label ) {
							echo ' no-label';
						}
		...
		<?php endif; ?>
			...
			echo 'System report was not configured!';
		endif;
	}
	...
	public function render_text() {
		...
		if ( $boxes ) {
			...
			foreach ( $boxes as $box ) :
				...
				if ( ! empty( $box['items'] ) ) {
					foreach ( $box['items'] as $item ) {
						...
						$data = $this->get_item_data( $item );
						if ( $data ) {
							ak_sanitize_echo( $data[1] ); // escaped before
						} else {
							esc_html_e( 'Nothing!', 'ak-framework' );
						}
					}
				}
				echo "\n";
			...
			echo '### ', __( 'Error Log', 'ak-framework' ), ' ###', "\n";
			print_r( get_option( 'ak-backend-error-log' ) );
		} else {
			esc_html_e( 'System report was not configured!', 'ak-framework' );
		}
	...
	protected function get_report_settings() {
		...
		$file = $this->page['config']['file'];

		if ( ! isset( $file ) || ! file_exists( $file ) ) {
			throw new \Exception( 'There is no field config file for this panel!' );
		}
		...
		switch ( $box['box-settings']['operation'] ) {
			case 'list-active-plugin':
				...
				if ( $plugins ) {
					foreach ( (array) $plugins as $plugin ) {
						...
						$plugin_name = isset( $plugin['Name'] ) ? $plugin['Name'] : __( 'unknown', 'ak-framework' );

						$author_uri  = isset( $plugin['AuthorURI'] ) ? esc_url( $plugin['AuthorURI'] ) : '#';
						$author_name = isset( $plugin['Author'] ) ? $plugin['Author'] : __( 'unknown', 'ak-framework' );

						if ( $this->render_context === 'html' ) {
							$box['items'][] = array(
								'type'        => 'raw',
								'label'       => sprintf( '<a href="%s" target="_blank">%s</a>', $plugin_uri, $plugin_name ),
								'description' => wp_kses(
									sprintf(
										/* translators: 1: plugin url 2: plugin name */
										__( 'by <a href="%1$s" target="_blank">%2$s</a>', 'ak-framework' ), $author_uri, $author_name
							...
							$box['items'][] = array(
								...
								'description' => sprintf(
									...
									__( 'by %1$s (V %2$s)', 'ak-framework' ), $author_name, $plugin_version
								),
							);
						}
					}
					...
					$box['items'][] = array(
						...
						'label'       => false,
						'count_calc'  => false,
						'description' => sprintf( '<div class="ak-product-notice ak-product-notice-warning">%s</div>', __( 'no active plugin was found!', 'ak-framework' ) ),
					);
				}
			...
			case 'report-export':
				if ( 'html' === $this->render_context ) {
					$box['items'][] = array(
						'type'        => 'raw',
						'label'       => sprintf( '<a href="#" class="bf-btn-primary ak-pages-primary-btn ak-pages-success-btn" id="ak-get-system-report"><span class="loading" style="display: none;margin: 0 5px;"><i class="fa fa-refresh fa-spin"></i></span> %s</a>', __( 'Get Status Report', 'ak-framework' ) ),
						'description' => __( 'Click the button to produce a report, then copy and paste into your support ticket.', 'ak-framework' ),
					);
					$box['items'][] = array(
						'type'        => 'raw',
						'label'       => false,
						'class'       => 'ak-item-hide',
						'description' => '<div id="ak-system-container" style="display: none;"><textarea rows="20" style="width: 100%;color: #595959;" class="ak-output">' . $this->render_text() . '</textarea><a href="#" class="bf-btn-primary" id="ak-copy-system-report">' . __( 'Copy status report', 'ak-framework' ) . '</a></div>',
					);
				}
	...
	protected function get_wp_data( $data_type ) {
		...
		switch ( $data_type ) {
			...
			case 'memory_limit':
				...
				if ( $this->_is_ini_value_changeable( 'memory_limit' ) &&
			...
			case 'cache_exists':
				...
				foreach ( $cache_plugins as $cache_plugin => $plugin_name ) {
					if ( function_exists( WP_PLUGIN_DIR . '/' . $cache_plugin ) ) {
	...
	protected function test_http_remote() {
		$prev_status = get_transient( 'ak_remote_test' );
		...
		set_transient( 'ak_remote_test', $new_status, $this->check_remote_duration );
	...
	protected function help_section_html( &$data ) {
		...
		if ( ! empty( $this->active_item['help'] ) ) {
			?>
<div class="ak-pages-help-wrapper">
			<?php
				$icon_classes     = array( 'fa' );
			$icon_wrapper_classes = array( 'ak-pages-help' );
	...
	<div class="ak-pages-help-description">
			<?php
			if ( is_string( $this->active_item['help'] ) ) {
				ak_sanitize_echo( $this->active_item['help'] );
			} // escaped before
			?>
	</div>
</div>
	...
	protected function item_section_html( &$data ) {
		?>
		<span class="ak-item-description">
		<?php

		if ( isset( $this->active_item['before_description'] ) ) {
			ak_sanitize_echo( $this->active_item['before_description'] ); // escaped before
		}

		ak_sanitize_echo( $data[1] ); // escaped before

		if ( isset( $this->active_item['after_description'] ) ) {
			ak_sanitize_echo( $this->active_item['after_description'] ); // escaped before
		}
		?>
		</span>
		<?php
	 ...
	 * @return string
	 */
	private function get_ak_pages_data( $data_type ) {
		$result = '';
		switch ( $data_type ) {
			case 'history':
				/*
				 ...
				 */
				if ( $imported_demo = get_option( '_ak_current_demo_id_' ) ) {
					$result = sprintf( 'Current Demo: %s', $imported_demo );
				} else {
					$result = __( 'Nothing!', 'ak-framework' );
				}
	...
	private function sanitize_item( $result, $data_type, $context = 'raw' ) {
		...
		switch ( $context ) {
			case 'display':
				if ( is_bool( $result ) ) {
					$return = $result ? __( 'Enabled', 'ak-framework' ) : __( 'Disabled', 'ak-framework' );
	 ...
	 */
	function _is_ini_value_changeable( $setting = 'memory_limit' ) {
