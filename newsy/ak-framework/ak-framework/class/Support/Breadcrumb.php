<?php
/**
 * @modified by akbilisim - changed css classes and hook names
 *
...
class Breadcrumb {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'ak-framework/breadcrumb/args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="ak-breadcrumb-items" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = 'ak-breadcrumb-item';
				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' ak-breadcrumb-begin';
				} elseif ( $item_count === $item_position ) {
					$item_class .= ' ak-breadcrumb-end';
				}
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="ak-breadcrumb breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'ak_breadcrumb', $breadcrumb, $this->args );
		if ( false === $this->args['echo'] ) {
			return $breadcrumb;
		}

		ak_sanitize_echo( $breadcrumb );
	}
	...
	protected function set_labels() {
		$defaults     = array(
			'browse'              => ak_get_translation( 'Browse:', 'ak-framework', 'bread_browse' ),
			'aria_label'          => ak_get_translation( 'Breadcrumbs', 'ak-framework', 'breadcrumbs' ),
			'home'                => ak_get_translation( 'Home', 'ak-framework', 'bread_home' ),
			'error_404'           => ak_get_translation( '404 Not Found', 'ak-framework', 'bread_404_not_found' ),
			'archives'            => ak_get_translation( 'Archives', 'ak-framework', 'bread_archives' ),
			// Translators: %s is the search query.
			'search'              => ak_get_translation( 'Search results for: %s', 'ak-framework', 'bread_search_results' ),
			// Translators: %s is the page number.
			'paged'               => ak_get_translation( 'Page %s', 'ak-framework', 'bread_page' ),
			// Translators: %s is the page number.
			'paged_comments'      => ak_get_translation( 'Comment Page %s', 'ak-framework', 'bread_comment' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => ak_get_translation( 'Minute %s', 'ak-framework', 'bread_minute' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => ak_get_translation( 'Week %s', 'ak-framework', 'bread_week' ),
		...
		$this->labels = apply_filters( 'ak-framework/breadcrumb/labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'ak-framewok/breadcrumb/post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'ak-framework/breadcrumb/items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'ak-framework' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'ak-framework' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'ak-framework' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ak-framework' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ak-framework' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'ak-framework' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ak-framework' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'ak-framework' ) ) );
		// Add the year item.
		$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $year );
		// Add the week item.
		if ( is_paged() ) {
	 ...
	 * @since  0.6.0
	 *
	 * @param integer $slug The post type archive slug to search for.
	 *
	 * @return void
	 */
	 ...
	 * @since  1.0.0
	 *
	 * @param integer    $post_id  The ID of the post to get the terms for.
	 ...
	 * @since  1.0.0
	 *
	 * @param integer    $term_id  ID of the term to get the parents of.
	 ...
	 * @since  0.6.0
	 *
	 * @param integer    $post_id ID of the post whose parents we want.
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ak-framework' ) ) ) );
				}
				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ak-framework' ) ) ) );
				}
				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'ak-framework' ) ) ) );
				}
